...
for i = 1:T
    ...
    V = [1,x1.*cos(theta) - y1.*sin(theta),x1.*sin(theta) + y1.*cos(theta);
        1, x2.*cos(theta) - y2.*sin(theta),x2.*sin(theta) + y2.*cos(theta);
        1, x3.*cos(theta) - y3.*sin(theta),x3.*sin(theta) + y3.*cos(theta)];    
